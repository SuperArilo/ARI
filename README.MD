...
| 🛍️ `背包检查编辑`       | 可检查编辑指定玩家(或离线)的背包 `/inv <player_name>`              |
| 🛠️ `服务器维护模式`      | 启动服务器维护模式 `/maintenance`                            |
| 🚵🏻‍♀️ `AFK 模式`   | 开启自己或指定玩家的 AFK 模式 `/AFK <player_name>`              |
| 🫥 `全局隐身`          | 开启全局隐身模式来观察服务器里的玩家 `/vanish`                        |
| 👻 `自定义玩家的前后缀名称展示` | `/nick` 指令再配合 `PlaceholderAPI` 占位符来自定义展示自定义玩家的前后缀名称 |
## 🗯️ 权限列表
| 功能                              | 描述                  |
|---------------------------------|---------------------|
...
| `ari.command.enderchest`        | 快捷打开自己的末影箱          |
| `ari.command.enderchest.player` | 打开指定玩家的末影箱          |
| `ari.command.inv`               | 可查看和编辑指定玩家的背包       |
| `ari.command.afk`               | 玩家可自己开启 AFK 模式      |
| `ari.command.afk.player`        | 开启指定玩家的 AFK 模式      |
| `ari.command.vanish`            | 开启全局隐身模式            |
| `ari.command.reload`            | 重载插件配置              |
| `ari.command.maintenance`       | 启动服务器维护模式           |
| `ari.command.nick`              | 设置指定玩家昵称的前后缀        |
| `ari.command.tp`                | 使用插件来进行指定玩家传送       |
| `ari.command.gamemode`          | 使用插件来指定调整自己的游戏模式    |
...
| `ari.skip-command-cooldown`     | 可跳过指令冷却时间限制         |
| `ari.pass-spam`                 | 为指定玩家跳过滥发消息被踢出服务器   |
| `ari.pass-afk`                  | 可将指定玩家或组永不处于 AFK 模式 |

## 🕮 Placeholder 变量列表

| 功能                         | 描述         |
|----------------------------|------------|
| `%ari_player_name_prefix%` | 自定义玩家的前缀前缀 |
| `%ari_player_name_suffix%` | 自定义玩家的后缀名  |
