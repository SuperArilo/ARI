...
public class CoolDownStateService extends StateService<CooldownState> {
    ...
    protected boolean canAddState(CooldownState state) {
        ...
        if (Ari.PERMISSION_SERVICE.hasPermission(owner, "ari.cooldown." + state.getType().getKey())) return false;
    ...
    protected void abortAddState(CooldownState state) {
        Ari.instance.getLog().debug("player {} has permission skip cooldown.", state.getOwner().getName());
    }
